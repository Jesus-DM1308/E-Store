...
export class UpdateUserDto {
  ...
  static create(props: { [key: string]: any }): [string?, UpdateUserDto?] {
    ...
    const nameRegex =
      /^[A-ZÁÉÍÓÚÑa-zñáéíóúü][ ]?[A-ZÁÉÍÓÚÑa-zñáéíóúü]+(?:[ ]?[A-ZÁÉÍÓÚÑa-zñáéíóúü]+)*$/;
    ...
    if (password) {
      // minimo    MAY        min         num          caracter        8 caracteres
      const passRegex =
        /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/;
    ...
    return [
      '',
      new UpdateUserDto(id, name, last_name, email, password, cel, user_type),
    ];
  }
}
