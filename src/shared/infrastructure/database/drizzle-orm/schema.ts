...
export const ordersTable = pgTable(`order`, {
  id: integer(`id`).primaryKey().generatedAlwaysAsIdentity().notNull(),
  statusId: integer(`status_id`).notNull().references(() => statusOrderTable.id, { onDelete: 'restrict'}),
...
export const orderDetailTable = pgTable(`order_detail`, {
  ...
  productUserId: integer(`product_user_id`).notNull().references(() => productsUsersTable.id, { onDelete: 'restrict' }),
